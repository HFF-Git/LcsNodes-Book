...
As a consequence, the library is not exactly a small veneer on top of the hardware and does take its program memory toll on controller storage. However, with the growing capabilities of modern controllers, this should not be a great limitation. 
...
\unnumberedSubSection{Node and Port Items}
...
\begin{table}[!ht]
    \begin{center}
        ...
        \begin{tabularx}{0.9\textwidth}{|l|l|X|}
            ...
            0 & & NIL Item \\
            \hline
            1 & 63 & Reserved items for node and ports with defined names. For the Get and SET access the item refers to an attribute variable. For the REQ access it refers to a user defined function to be invoked via a callback. \\
            \hline
            64 & 127 & Reserved items for node and ports functions.The REQ access refers to a user defined function to be invoked via a callback. \\
            \hline
...
The item 64 to 127 represent the user defined functions. Only a REQ call uses these items. A callback function is invoked.
...
\unnumberedSubSection{Pending Request Map}
