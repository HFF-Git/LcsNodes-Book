...
firmware code. It is not intended to replace the LCS message protocol. Instead, it
...
sufficient to communicate with the node. The serial interface provides two related 
...
\begin{lstlisting}
uint8_t lcsMsgToStr( char *msgStr,
                     uint8_t *dataBuf,
                     uint8_t dataBufLen );

uint8_t strToLcsMsg( uint8_t *dataBuf,
                     uint8_t dataBufLen,
                     char *msgStr );
\end{lstlisting}
...
The \texttt{s} command accepts a numeric parameter which indicates what status 
information to print. They are grouped for showing memory data, NVM data and a 
formatted version of some data.
...
runtime requests, event handling and message processing. 

This makes it a valuable tool
