...
The CDC layer provides only a thin abstraction. It hides the details
...
The resource abstraction is not limited to simple controller
...
\begin{tikzpicture}[scale=0.9, transform shape]
    ...
    % Descriptors
    \node[tsCell] at (2.2,7.5) {\textbf{Descriptors}}; 
    \node[tsCell] at (2.2,0.5) {Board Type};
	\node[tsCell] at (2.2,1.5) {Board Version};
	\node[tsCell] at (2.2,4.0) {Descriptor Map};
    \draw (2,1)--(7,1);
    \draw (2,2)--(7,2);

     % Resources
     \node[tsCell] at (10.2,7.5) {\textbf{Resources}};
     \node[tsCell] at (10.2,0.5) {Board Descriptor};
     \node[tsCell] at (10.2,5) {Resource Map};
...
The first eight resource numbers are reserved for the runtime library itself. They
...
The runtime library handles the configuration of these resources at startup.
...
servo drivers or track occupancy detectors. Such logical resources
...
%--------------------------------------------------------------------------------
\unnumberedSection{Configuration Validation}
...
The CDC library is initialized using the resource descriptor map. Although
the firmware programmer can explicitly initialize the libraey, the LCS runtime
library does it also at runtime initialization. 
Explicitly initializing is helpful if the firmware needs to access hardware
resources before initializing the LCS runtime.
...
identifier can be used whenever an LCS node requires a hardware unique Id
or serial number.
...
uint8_t         getUartBuffer( uint8_t rNum, 
                               uint8_t *buf, 
                               uint8_t bufLen );
\end{lstlisting}
\FloatBarrier
...
instance. Additional utility functions support bus reset and automatic device
...
??? should we add send and receive calls to this interface ?

%--------------------------------------------------------------------------------
\unnumberedSection{Programmable State Machines}

The Raspberry Pi Pico offers Programmable I/O (PIO) state machines that are 
particularly well suited for communication protocols and precise signal generation.
...
LCS uses the CAN bus as its communication backbone. From a hardware perspective,
microcontrollers either provide a CAN controller as an on-chip peripheral or 
rely on an external CAN controller such as the MCP2515. The RP2040 offers an 
interesting third alternative. Its Programmable I/O (PIO) state machines are 
powerful enough to implement the CAN protocol entirely in software, requiring 
only an external CAN bus transceiver.

The Pico implementation of LCS uses the can2040 library. It is a little gem 
of a library that implements the complete CAN protocol using the RP2040 PIO 
hardware. Apart from the CAN transceiver, no dedicated CAN controller is 
required. Since the RP2040 provides two processor cores, one core is dedicated 
to CAN communication while the other executes the application firmware.

\unnumberedSubSection{H-Bridge Control}

We will also make extensive use of the RP2040 PIO state machines in the block 
controller. Here they replace a considerable amount of discrete logic that 
would otherwise be required to drive the H-Bridges, generate the DCC waveform, 
manage the RailCom cutout periods, and detect incoming RailCom datagrams. 
Besides simplifying the hardware design, the PIO implementation keeps all 
timing-critical signal generation in dedicated hardware.

\unnumberedSubSection{DCC Signal Generation}

One could also imagine generating the DCC waveform directly in a PIO state 
machine. Such an implementation would move the precise waveform timing entirely 
into hardware and would further reduce the real-time requirements placed on 
the firmware.

The current LCS implementation follows a different approach. 
DCC packet generation is implemented as a software state machine running in 
the base station firmware. This provides excellent flexibility, keeps the 
implementation easy to understand and debug, and has proven more than capable 
of meeting the timing requirements. The resulting bit stream is then 
transferred to the hardware that drives the track output.

A future version of the base station may move the waveform generation into 
a PIO state machine, in much the same way as the CAN bus interface and H-bridge 
control logic already benefit from programmable I/O. At present, however, 
the software implementation is straightforward, reliable, and fully satisfies 
...
runtime library abstracts the communication and node behavior, while the CDC
