\chapterr{\textit{Hardware Extension Module Design}}
...
\section{\textit{Extension library}}
...
struct LcsCoreLib {
   ...
   uint8_t drvInfo( uint8_t boardId, uint8_t padId, uint8_t item, uint16_t \textit{arg1, uint16_t }arg2 = nullptr );
   uint8_t drvRead( uint8_t boardId, uint8_t padId, uint16_t *arg );
   uint8_t drvWrite( uint8_t boardId, uint8_t padId, uint16_t arg );
   uint8_t drvRead( uint8_t boardId, uint8_t padId, uint8_t \textit{buf, uint8_t }len );
...
\section{\textit{Extension Descriptor Memory}}
...
\begin{itemize}
\item end point entry structure ( 8 bytes )
\item device type ( DIO out, DIO IN/PullUp, DIO general I/O,  AnalogIn, PWM, Servo, ...)
\item flags
\item initial value / state
\item I2C address
\end{itemize}
...
\section{\textit{Extension Board Driver}}
...
struct LcsDrv {
   ...
   public:
      ...
      virtual uint8_t info( uint8_t padId, uint8_t item, uint16_t \textit{arg1, uint16_t }arg2 = nullptr ) = 0;
      virtual uint8_t read( uint8_t padId, uint16_t *arg ) = 0;
      virtual uint8_t write( uint8_t padId, uint16_t arg ) = 0;

      virtual uint8_t read( uint8_t padId, uint8_t \textit{buf, uint8_t }bufLen ) = 0;
...
\section{\textit{Utility for writing the NVM}}
...
// ??? \textbf{note}  to be decided and implemented. I like the idea for simple commands on the core library level... easiest way to move quickly forward.
