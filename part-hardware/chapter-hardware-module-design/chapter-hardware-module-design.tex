...
The external I2C connector is available both on the controller board. 
Socket extension boards typically use this interface as their primary communication channel with the 
controller board.
In addition, the I2C bus is also available on the extension board connector.
...
The Raspberry Pi Pico provides a very flexible pin assignment system. With few execptions, 
many peripheral functions can be mapped to different GPIO pins. 
The extension connector therefore exposes a carefully selected set 
...
For example, a turnout or signal may be located one or two meters away from the main controller. Running 
many individual wires back to a central extension board would not be practical.
For these cases, small \textbf{socket extension board}s are used.
...
The extension board provides the more powerful local hardware resources, while socket boards 
provide smaller dedicated functions. For example, a signal socket extension board, would 
manage two signals and is located close top where the signals are on the layout. 
...
The I2C bus connects the socket board to the controller board nearby, the controller boards
themselves communicate via the LCS bus and are spread throughout the layout.
Since only a limited number of extension boards are normally connected to one controller, the signal 
line lengths of the I2C bus remain reasonable. 
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1, transform shape]
        ...
        \node at (7.5, 2) {\includegraphics[page=1, scale=0.7]
				{./part-hardware/chapter-hardware-module-design/LCS-FP-SOCKET-5X8.pdf}};  	
