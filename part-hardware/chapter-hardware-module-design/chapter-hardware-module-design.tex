...
\unnumberedSection{The Controller Platform}
 ...
 The following table gives some guidance on the capabilities typically needed in the LCS controller designs. This list also applies in general to other controllers.
	...
	\begin{tikzpicture}[scale=0.9, transform shape]
    	...
    	\node at (8,4) {
        	\includegraphics[page=1, scale=0.7]
				{./part-hardware/chapter-hardware-main-module-design/LCS-FP-MAIN-10X12.pdf}
    	};
...
All boards will have a form factor of 10cm wide and 8cm, 12cm, 16cm and 20cm long. The figure above shows the 10cm by 12cm board form factor. The main controller board has on the left side the connectors for the LCS bus, the external I2C bus and the power input. On the right side, there is one connector toward an extension board. This connector is optional.

To ease the hardware schematic development and ensure that all boards fit together, the PCB boards along with their connectors are available as symbols and PCB footprints in the EasyEDA library.
...
The external I2C line, called the \textbf{I2C bus}, is exposed to the extension boards as well as a connector next to the LCS bus. As we will see in the part on extension boards, extension boards are mainly accessed via the I2C bus. 
...
The extension connector is comprised of three connectors. In the middle is a 40-pin connector with power distribution, communication and GPIO pins. All main controller boards have this connector. In addition, there are two 8-pin connectors for the track power lines. They are only used by the base station and the block controller.
...
??? to ensure  that main controller and extension board fit... 
...
\unnumberedSection{Controller, Extension and Socket Board}

??? the big picture perhaps earlier ...
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1, transform shape]
		...
		\node at (12, 7) {\includegraphics[page=1, scale=0.5]
				{./part-hardware/chapter-hardware-main-module-design/LCS-FP-EXT-10X12.pdf}};
        ...
        \node at (4.5, 6) {Extension Bus};
        \draw[line width=1mm, gray!40, ->] (6,6) -- (10,6);
        
        \node at (13.5, 1.5) {\textbf{Socket Boards}};
        
        \node at (3.5, 1.5) {\includegraphics[page=1, scale=0.35,  angle=270]
				{./part-hardware/chapter-hardware-main-module-design/LCS-FP-SOCKET-5X8.pdf}};
...
// ??? connected via I2C bus which delivers power and the I2C communication

??? intended for specialized local functions close to the managed entity.

??? example a socket extension for driving a turnout or a signal
...
??? this was a key chapter on how we go  about building hardware ...
