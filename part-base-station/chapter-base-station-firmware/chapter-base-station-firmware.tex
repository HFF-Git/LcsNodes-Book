\unnumberedChapter{Base Station Firmware}
...
The base station is a rather complex node. Especially the DCC signal generation requires the node to be very close to the controller chip hardware capabilities. The individual parts will be described in the following sections. From an overall firmware perspective, the base station does its work in a functions that is called periodically from the LCS core library. The difference to other nodes is that the key work of producing the DCC signal is interrupt driven. There is a central timer that interrupts whenever the signals need to change their value is the central heartbeat of the node. The same is the case for the RailCom UART input. Everything that is part of the interrupt driven signal path will also just use interrupts for asynchronous execution. All else takes place outside the interrupt routines and is rather straightforward.

The base station module firmware is divided into a couple of key software modules. The section that follow present a high level overview and review some of the design choices taken. The source files contains an extensive documentation how this all works. 
...
\begin{longtable}{@{}|l|l|l|p{0.5\linewidth}|@{}}
    ...
    \toprule
    \textbf{Item} & \textbf{Arg0} & \textbf{Arg1} & \textbf{Purpose} \\
    \midrule
    \endfirsthead
    \toprule
    \\textbf{Item} & \textbf{Arg0} & \textbf{Arg1} & \textbf{Meaning} \\
    \midrule
    \endhead
    \midrule
...
\unnumberedSection{DCC Track Signal Management}
...
\unnumberedSection{DCC Track Power Measurement}
...
\unnumberedSection{Track Power Management}
...
\unnumberedSection{Base Station Items}
...
\unnumberedSection{RailCom Support Attributes and Functions}
...
\unnumberedSection{Base Station Dictionary}
...
\unnumberedSection{Base Station Serial Commands}

The base station, like any other LCS node, features a serial command interface. We know it so far as an interface to look at LCS data structures and issue simple commands to a node for testing and debugging. One of the LCS library capabilities is to invoke a command callback when we get a non-LCS command. This feature  is used by the base station to implement a command interface. 

Naturally, one will not sit in front of a computer and type in the rather cryptic commands for controlling an engine. However, serial commands are an excellent interface to other software such as the JMRI tools suite or Layout control programs such as RocRail. They two named offers an interface based on the \texttt{DCC++ / DCC-EX} command syntax. Given our base station offers these commands, we can easily integrate with such systems. The command interface is quite important, so we will have a separate chapter coming up next on this topic.
