...
The query node \texttt{(NODE-GET)} and node reply messages \texttt{(NODE-REP)} are available to obtain attribute data from the node or port. The \texttt{(NODE-SET)} allows to set attributes for a node or port on the targeted node. The \texttt{(NODE-REQ)} message passes a requests to the target node. Again, the \texttt{(NODE-REP)} is the reply message. The reply message can return one or two arguments.
