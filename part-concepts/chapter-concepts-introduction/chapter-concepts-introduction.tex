...
The software architecture should nevertheless be able to manage thousands of modules. Although the underlying transport technology may eventually change, the LCS message format, maximum message size, and broadcast communication model are part of the system design and remain stable.
...
Callbacks are a central part of this model. A typical firmware initializes the runtime, which in turn initializes the lower layers. It then registers callback routines for the runtime events relevant to the module. Finally, the firmware starts the runtime. From that point, the runtime receives messages, performs periodic tasks, and invokes the registered callbacks whenever the module must respond.
