%-------------------------------------------------------------------------------------------------------
%-------------------------------------------------------------------------------------------------------
\unnumberedChapter{Message Protocols}
...
\unnumberedSection{Node startup}
...
\unnumberedSection{Switching between Modes}
...
\unnumberedSection{Setting a new Node Id}
...
\unnumberedSection{Node and Port Reset}
...
\unnumberedSection{Node and Port Access}
...
\unnumberedSection{Layout Event management}
...
\unnumberedSection{General LCS Bus Management}
...
\unnumberedSection{Locomotive Configuration Management}
...
\unnumberedSection{Configuration Management using RailCom}
...
\unnumberedSection{Sending DCC packets}

The base station is the hardware module that receives the LCS messages for configuring and running locomotives. The primary task is to produce DCC signals to send out to the track. Besides locomotive commands, the DCC standard defines more messages. For example, stationary devices such as a turnout with a DCC encoder, can directly be addressed. In addition to controlling locomotives, the base station also offers to just send out raw DCC packets.
...
Work into the document...

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|r|r|}
\hline
\textbf{Payload} & \textbf{Bits/frame} & \textbf{Baud rate} & \textbf{Max msg/s} & \textbf{Safe msg/s} \\
\hline
5 B (event) & 128 & 125 kbit/s & 977 & 480 \\
            &     & 250 kbit/s & 1953 & 975 \\
\hline
6 B (request/reply) & 138 & 125 kbit/s & 906 & 450 \\
                    &     & 250 kbit/s & 1811 & 900 \\
\hline
8 B (worst case) & 157 & 125 kbit/s & 796 & 400 \\
                 &     & 250 kbit/s & 1599 & 800 \\
\hline
\end{tabular}
\label{tab:can_bus_capacity}
\caption{CAN Bus Frame Capacity for 29-bit IDs}
\end{table}