...
// ??? rework the text, what are concepts ?

// ??? extensions and satellites fit nicely into the many sensors and actors we will see...
...
With so many sensors and actors, a key requirement is to implement a concept where most of the functional parts can be used in different combinations. The chapter on hardware design already presented the main controller and extensions concept. A key reason for this concept was exactly the large variety of sensors and actors.
...
\unnumberedSection{Controller Examples}
...
\unnumberedSection{Extension Examples}
...
\item \textbf{GPIO}. The GPIO extension boards is a universal digital input / output board. It is typically used for input such as switches or push buttons a well as LEDs connected. Care has to be taken to match the number of output consumers with the what the individual output pins can drive.
