\unnumberedChapter{The Cab Handheld}
...
But how about a cab handheld that just feature instead of all the functions for controlling an entire layout just the features for controlling an engine. Our cab handheld will have dedicated buttons and levers for let's say a horn or whistle, a bell, and so on. There are also configuration buttons, dedicated buttons and switches, and a very small set of buttons to map to loco specific functions. Furthermore, there is of course the rotary knob for setting the locomotive speed. The following figure shows a rough sketch of the cab handheld elements.
...
Configuration and part of operation takes place with four buttons, which surround the screen display. The \texttt{MENU} button allows to toggle through the menus defined. To select a menu, the \texttt{SELECT} button is used. The menu toggle and select scheme can be nested. Within a menu screen, the \texttt{MENU}, \texttt{UP} and \texttt{DOWN} buttons are used screen specific and the \texttt{SELECT} button typically confirms the selected action. The direction buttons \texttt{REV} and \texttt{FWD} and the \texttt{SPEED} knob set the speed and direction of the locomotive or consist. \texttt{F1} to \texttt{F4} are four general buttons that can be mapped to special functions of the particular locomotive. The Horn and Bell button are rounding up the initial design.

The screen itself has also a common structure for all data displayed. The corner field match the navigation buttons next to the corner. The text shown is screen dependent. Typically the action of the four buttons is shown. Between the two corner fields on the top and on the button, there is a data field with general status information. Finally, there are two context dependent screen lines in the center of the screen. 
...
There is certainly the option to connect commercially available handhelds. This would require to provide a gateway from let's say a LocoNet protocol based handheld to the LCS protocol. Well, one day you should be able to connect such handhelds via the LocoNet bus. But right now the topic is on the backlog list. It is a subject for the Gateway part of this book.
